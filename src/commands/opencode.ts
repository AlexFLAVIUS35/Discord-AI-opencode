...
export const opencode: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (isUserInstallOnly) {
      // User-installed apps cannot use the bot's guild channel API. The
      // interaction response/follow-up webhook is the authorized way to post
      // back into the exact server context where the command was invoked.
      const conversationId = interaction.channelId;
      await interaction.deferReply();
      await runPrompt(null, conversationId, prompt, conversationId, interaction.user.id, interaction);
      return;
    }
