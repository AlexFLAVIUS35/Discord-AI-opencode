...
export const session: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('view')
        .setDescription('View info for the session attached to this thread')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'list') {
      ...
      try {
        ...
        await interaction.editReply({ content: `❌ Failed to list sessions: ${(error as Error).message}` });
      }
      return;
    }
    ...
    if (subcommand === 'attach') {
      ...
      if (!thread?.isThread()) {
        await interaction.reply({ content: '❌ This command can only be used in a thread.', flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      const projectPath = dataStore.getChannelProjectPath(parentChannelId);
      if (!projectPath) {
        await interaction.editReply({ content: '❌ No project set for this channel. Use `/use <alias>` to set a project.' });
        return;
      }
      ...
      try {
        ...
        if (activeSessions.length === 0) {
          await interaction.editReply({ content: '❌ No sessions available to attach.' });
          return;
        }
        ...
        const select = new StringSelectMenuBuilder().setCustomId('session-attach-select').setPlaceholder('Select a session to attach').addOptions(options);
        const row = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(select);
        await interaction.editReply({ content: 'Select a session to attach to this thread:', components: [row] });
        ...
        collector.on('collect', async (i) => {
          ...
          if (!sessionInfo) {
            await i.editReply({ content: '❌ Selected session is no longer available.', components: [] });
            return;
          }
          ...
          await i.editReply({ content: `✅ **${title}** (\`${sessionId.slice(0, 8)}\`) attached to this thread.${note}`, components: [] });
        });
        ...
        await interaction.editReply({ content: `❌ Failed to attach session: ${(error as Error).message}` });
      }
      return;
    }
    ...
    if (subcommand === 'detach') {
      ...
      if (!currentSession) {
        await interaction.reply({ content: '❌ No session attached to this thread.', flags: MessageFlags.Ephemeral });
        return;
      }
      const sseClient = sessionManager.getSseClient(threadId);
      if (sseClient) { sseClient.disconnect(); sessionManager.clearSseClient(threadId); }
      sessionManager.clearSessionForThread(threadId);
      await interaction.reply({ content: '✅ Session detached from this thread.', flags: MessageFlags.Ephemeral });
      return;
    }
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Created At', value: threadSession ? formatRelativeTime(threadSession.createdAt) : 'unknown', inline: true },
            { name: 'Last Used At', value: threadSession ? formatRelativeTime(threadSession.lastUsedAt) : 'unknown', inline: true },
        ...
        await interaction.editReply({ content: `❌ Failed to read session info: ${(error as Error).message}` });
      }
    }
  },
};
