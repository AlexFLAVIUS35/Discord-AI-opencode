...
import { resolveCatalogModel } from "../commands/model.js";
...
function isDiscordAttachmentUrl(value: string): boolean {
  try { const url = new URL(value); const hostname = url.hostname.toLowerCase(); return (hostname === 'cdn.discordapp.com' || hostname === 'media.discordapp.net') && /^\/attachments\//i.test(url.pathname); } catch { return false; }
}
...
async function refreshDiscordAttachmentUrl(url: string): Promise<string | null> {
  ...
  if (!token) { console.error('[Media] Discord CDN refresh skipped: no DISCORD_BOT_TOKEN/DISCORD_TOKEN'); return null; }
  try {
    const response = await fetch('https://discord.com/api/v10/attachments/refresh-urls', { method: 'POST', headers: { Authorization: `Bot ${token}`, 'Content-Type': 'application/json', 'User-Agent': 'Leeha/1.0 (+https://github.com/AlexFLAVIUS35/Discord-AI-opencode)' }, body: JSON.stringify({ attachment_urls: [url] }), signal: AbortSignal.timeout(7000) });
    if (!response.ok) { console.error(`[Media] Discord CDN refresh failed: ${response.status} ${response.statusText}`); return null; }
    const data = await response.json() as { refreshed_urls?: Array<{ original?: string; refreshed?: string }> };
    return data.refreshed_urls?.find(item => item.original === url)?.refreshed ?? data.refreshed_urls?.[0]?.refreshed ?? null;
  } catch (error) { console.error('[Media] Discord CDN refresh error:', error instanceof Error ? error.message : error); return null; }
}

function isDiscordGifUrl(value: string): boolean { try { const url = new URL(value); return (url.hostname.toLowerCase() === 'cdn.discordapp.com' || url.hostname.toLowerCase() === 'media.discordapp.net') && /\.gif$/i.test(url.pathname); } catch { return false; } }

async function discordGifToPng(response: Response): Promise<{ url: string; mime: string } | null> {
  try { const contentLength = Number(response.headers.get('content-length') ?? 0); if (contentLength > MAX_MEDIA_BYTES) return null; const bytes = Buffer.from(await response.arrayBuffer()); if (bytes.byteLength > MAX_MEDIA_BYTES) return null; const png = await sharp(bytes, { pages: 1, page: 0 }).png().toBuffer(); if (png.byteLength > MAX_MEDIA_BYTES) return null; return { url: `data:image/png;base64,${png.toString('base64')}`, mime: 'image/png' }; } catch (error) { console.error('[Media] Failed to convert Discord GIF to PNG:', error instanceof Error ? error.message : error); return null; }
}

function isKlipyPageUrl(value: string): boolean { try { const url = new URL(value); return url.hostname.toLowerCase() === 'klipy.com' && /^\/gifs\//i.test(url.pathname); } catch { return false; } }
function klipySlug(value: string): string | null { try { const url = new URL(value); const match = url.pathname.match(/^\/gifs\/([^/?#]+)/i); return match?.[1] ? decodeURIComponent(match[1]) : null; } catch { return null; } }
function findKlipyMediaUrl(value: unknown): string | null { if (!value || typeof value !== 'object') return null; if (Array.isArray(value)) { for (const item of value) { const found = findKlipyMediaUrl(item); if (found) return found; } return null; } const object = value as Record<string, unknown>; for (const key of ['gif', 'mediumgif', 'tinygif', 'nanogif', 'url']) { const candidate = object[key]; if (typeof candidate === 'string' && /^https:\/\/static\d?\.klipy\.com\/.+\.(?:gif|webp|png|jpe?g)(?:[?#].*)?$/i.test(candidate)) return candidate; if (candidate && typeof candidate === 'object') { const found = findKlipyMediaUrl(candidate); if (found) return found; } } for (const candidate of Object.values(object)) { const found = findKlipyMediaUrl(candidate); if (found) return found; } return null; }

async function resolveKlipyGif(url: string): Promise<{ url: string; mime: string } | null> {
  const appKey = process.env.KLIPY_API_KEY ?? process.env.KLIPY_APP_KEY; const slug = klipySlug(url); if (!appKey || !slug) { console.error(`[Media] KLIPY link needs KLIPY_API_KEY/KLIPY_APP_KEY: ${url}`); return null; }
  try { const endpoint = new URL(`https://api.klipy.com/api/v1/${encodeURIComponent(appKey)}/gifs/items`); endpoint.searchParams.set('slugs', slug); const response = await fetch(endpoint, { headers: { 'User-Agent': 'Leeha/1.0', Accept: 'application/json' }, signal: AbortSignal.timeout(7000) }); if (!response.ok) { console.error(`[Media] KLIPY API failed for ${url}: ${response.status} ${response.statusText}`); return null; } const data: unknown = await response.json(); const mediaUrl = findKlipyMediaUrl(data); if (!mediaUrl) return null; const mediaResponse = await fetchExternal(mediaUrl); if (!mediaResponse || !mediaResponse.ok) return null; return responseToMedia(mediaResponse); } catch (error) { console.error('[Media] Failed to resolve KLIPY GIF:', error instanceof Error ? error.message : error); return null; }
}

async function resolveLinkedMedia(url: string): Promise<{ url: string; mime: string } | null> {
  if (isKlipyPageUrl(url)) { const klipyMedia = await resolveKlipyGif(url); if (klipyMedia) return klipyMedia; }
  const refreshedDiscordUrl = await refreshDiscordAttachmentUrl(url); const fetchUrl = refreshedDiscordUrl ?? url; const response = await fetchExternal(fetchUrl); if (!response || !response.ok) return null;
  const responseMime = response.headers.get('content-type')?.split(';')[0].toLowerCase();
  if (isDiscordGifUrl(fetchUrl) && responseMime === 'image/gif') return discordGifToPng(response);
  ...
  const contentLength = Number(response.headers.get('content-length') ?? 0); if (contentLength > 2 * 1024 * 1024) return null;
  const finalUrl = safeUrl(response.url || fetchUrl); if (!finalUrl) return null; const html = await response.text();
  for (const candidate of extractMetaMedia(html, finalUrl)) { const imageResponse = await fetchExternal(candidate); if (!imageResponse || !imageResponse.ok) continue; const media = await responseToMedia(imageResponse); if (media) return media; }
  return null;
}

export async function resolveLinkedGifAttachments(text: string): Promise<PromptMediaAttachment[]> {
  const urls = text.match(/https?:\/\/[^\s<>]+/gi) ?? []; const result: PromptMediaAttachment[] = []; const seen = new Set<string>();
  for (const rawUrl of urls.slice(0, 10)) { const url = rawUrl.replace(/[),.!?]+$/g, ''); if (seen.has(url)) continue; const resolved = await resolveLinkedMedia(url); if (!resolved) continue; seen.add(url); const extension = resolved.mime === 'image/gif' ? 'gif' : resolved.mime === 'image/webp' ? 'webp' : resolved.mime === 'image/png' ? 'png' : 'jpg'; result.push({ url: resolved.url, name: `linked.${extension}`, mime: resolved.mime }); }
  return result;
}

function decodeDataUrl(value: string): { bytes: Buffer; mime: string } | null { const match = value.match(/^data:([^;,]+);base64,(.+)$/s); if (!match) return null; const mime = match[1].toLowerCase(); if (!isSupportedImageMime(mime)) return null; try { const bytes = Buffer.from(match[2], 'base64'); if (bytes.byteLength > MAX_MEDIA_BYTES) return null; return { bytes, mime }; } catch { return null; } }
async function mediaParts(attachments: PromptMediaAttachment[]): Promise<{ type: string; mime: string; url: string }[]> { const parts: { type: string; mime: string; url: string }[] = []; for (const attachment of attachments.slice(0, 10)) { try { const decoded = decodeDataUrl(attachment.url); if (decoded) { parts.push({ type: 'file', mime: decoded.mime, url: attachment.url }); continue; } const response = await fetch(attachment.url); if (!response.ok) continue; const contentLength = Number(response.headers.get('content-length') ?? 0); if (contentLength > MAX_MEDIA_BYTES) continue; const bytes = new Uint8Array(await response.arrayBuffer()); if (bytes.byteLength > MAX_MEDIA_BYTES) continue; const mime = attachment.mime || response.headers.get('content-type')?.split(';')[0] || 'application/octet-stream'; if (!isSupportedImageMime(mime)) continue; parts.push({ type: 'file', mime, url: `data:${mime};base64,${Buffer.from(bytes).toString('base64')}` }); } catch (error) { console.error(`[Media] Failed to prepare attachment ${attachment.name}:`, error instanceof Error ? error.message : error); } } return parts; }
...
export async function sendPrompt(port: number, sessionId: string, prompt: string, model?: string, media: PromptMediaAttachment[] = [], agent?: string): Promise<void> {
  const parts: Array<{ type: string; text?: string; mime?: string; url?: string }> = []; const imageParts = await mediaParts(media); for (const part of imageParts) parts.push(part); parts.push({ type: 'text', text: prompt }); const body: Record<string, unknown> = { parts };
  if (model) { const resolved = resolveCatalogModel(model); if (resolved) body.model = resolved; }
  if (agent) body.agent = agent;
  const response = await fetch(`http://127.0.0.1:${port}/session/${encodeURIComponent(sessionId)}/prompt_async`, { method: "POST", headers: jsonHeaders(), body: JSON.stringify(body) });
  if (!response.ok) { assertNotAuthError(response.status, "Failed to send prompt"); throw new Error(`Failed to send prompt: ${response.status} ${response.statusText}`); }
}
